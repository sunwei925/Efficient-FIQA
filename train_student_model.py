...
from config_edgenet_linear_gfiqa import TrainingConfig
from FIQADatasetWithGFIQA import FIQADatasetWithGFIQA
import models.FIQA_model as FIQA_model
from utils import performance_fit, plcc_loss
...
def main():
    ...
    train_dataset = FIQADatasetWithGFIQA(
        data_dir=config.data_dir,
        gfiqa_data_dir=config.gfiqa_data_dir,
        csv_path=config.csv_path,
        gfiqa_csv_path=config.gfiqa_csv_path,
        transform=train_transform,
        is_train=True,
        train_ratio=config.train_ratio
    )
    
    val_dataset = FIQADatasetWithGFIQA(
        data_dir=config.data_dir,
        gfiqa_data_dir=config.gfiqa_data_dir,
        csv_path=config.csv_path,
        gfiqa_csv_path=config.gfiqa_csv_path,
    ...
    for epoch in range(config.num_epochs):
        ...
        # Validation
        val_loss, srcc, plcc = validate(model, val_loader, criterion1, criterion2, device, epoch, writer, logger)
        ...
        if current_score > best_score:
            ...
            logger.info(f"New best model saved at epoch {epoch+1} with score {current_score:.4f} (PLCC: {plcc:.4f}, SRCC: {srcc:.4f})")
        
    # Training completed, record total time
    total_time = time.time() - start_time
    logger.info(f"Training completed in {total_time/3600:.2f} hours")
    logger.info(f"Best Score: {best_score:.4f} (Epoch {best_epoch+1})")
